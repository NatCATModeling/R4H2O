...
The purpose of analysing data is to change reality positively. Changing the way we understand a problem enabling somebody to implement change are two ways in which we can achieve this purpose. Analysing data in RStudio is much fun, but hard to share with anyone who does not understand the language.

This chapter explains how to share the fruits of your labour with colleagues and other interested parties by generating reports that combine text and analysis through [literate programming](https://en.wikipedia.org/wiki/Literate_programming). Before we explain how to create reproducible reports, we delve into the data science workflow.
...
![Figure 6.1: Data science workflow.](resources/06_data_products/workflow.png)
...
{width: 60%}
![Summary of Rmarkdown to create presentations](https://youtu.be/9Uv3OOe-C2U)
...
The export format is listed in the first few lines of the markdown file. The example below shows how to define a title, author name and define the output as a Powerpoint presentation. This example also shows how to relate the output to a template (`reference_doc`).  This way, you can create presentations or reports using your organisation's templates.
...
You have now completed the first case study. The [next case study](#cleaning) discusses cleaning messy data from a customer survey and analysing their experiences.
